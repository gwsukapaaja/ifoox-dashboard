...
# Fungsi Membaca Data dari Firebase Cloud
...
def hitung_shelf_life(jenis, durasi_jam, suhu, gas, slope):
    ...
    faktor_gas = 1.8 if gas > 580 else (1.3 if gas > 360 else 1.0)
    faktor_slope = 1.3 if slope > 3.0 else (1.1 if slope > 1.0 else 1.0)
...
if st.button("🔍 Hitung Prediksi Shelf Life", type="primary"):
    ...
    for item in opsi_makanan:
        payload_dict[f"jenis_makanan_{item}"] = 1 if jenis_makanan == item else 0
    ...
    # LOGIKA PENGUNCI MONOTONIK (Anti-Naik)
...
if st.session_state["sudah_dianalisis"]:
    ...
    persentase_sisa = 0.0 if status_prediksi == "Tidak disarankan" else max(0.0, min(1.0, sisa_jam / max_jam))
    st.write("**Indikator Sisa Umur Simpan Makanan:**")
    st.progress(persentase_sisa)
else:
    st.info("💡 Klik tombol **'Hitung Prediksi Shelf Life'** untuk mulai analisis.")
